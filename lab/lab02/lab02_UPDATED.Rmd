...
### Instructions 
* Due date: Friday, September 3rd at 11:59pm PST.
...
Over the course of this lab and the first problem set, we will study the relationship 
...
button above. As you fill it out, you can re-knit (click the button again) and 
...
toggle `message = TRUE` to `message = FALSE` in the code chunk options. Re-knit your file and notice the difference in the output:
...
Look at the environment tab (top right pane) and make sure you see 
your dataset there. Click on `CS_data_raw` in that pane. This will open the 
...
you what each variable means. Below is an example of a dictionary for this 
dataset:
...
**Question 1. In your own words, what do these functions do?**
...
**Question 2. Based on these functions' outputs, who/what are the individuals in this dataset? How many of them are there?**
...
**Question 3. Right now, `CS_rate` is presented as a number between 0 and 1. For plotting, it will be nice to have the proportion displayed as a number between 0 and 100. We can use accomplish this using `dplyr`'s `mutate()` function to add a new variable called `CS_rate_100`. Write one line of code to add this variable to the dataset and assign this the new dataframe to an object called `CS_data`:**
...
**These countries represent a wide range of income levels and geographic regions.**
...
**Question 6. Because `Income_Group` and `Region` are categorical, we make bar charts to represent their distributions. Using the `ggplot()` code you learned during lecture, make a bar chart showing the number of countries per income group using `Income_Group`.**
...
**Question 7. Now make the bar chart using `Income_Group_order`.**
...
**Question 10. Make a bar chart showing the number of countries per geographic region.**
...
**Question 14. Rather than letting R choose the number of `bins` we can set `binwidth = number` within the `geom` function to choose the width of the bins. Try setting the binwidth to different values and see how it changes the plot.**
...
**Question 15. Next, make a histogram for cesarean delivery rates using the variable `CS_rate_100`. First run your code without a set binwidth and then add the `binwidth` argument with an appropriate value.** 
...
Sometimes, you need to save plots as a PNG or JPEG to include in a PowerPoint 
...
**Question 17. Use the documentation to attempt saving your plots in the code chunk below. Don't worry about producing an error or saving your plot in the wrong place -- that is all part of coding and learning new functions!**
...
cd; cd ph142-fa21/lab/lab02; python3 turn_in.py
