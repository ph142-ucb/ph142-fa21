...
you took a second sample and we both end up with samples of size 250.
...
The graph shows we all get fairly close when we take a sample of size 5000, but 
...
- The notation is important: S = {elements in the space} = (element 1, element 2, ... element n)
...
- We are interested in determining the probability (or chance or risk) that an event will occur. To calculate this probability, wee need to know the probabilities associated with the sample space.

### Proability model

- A probability model is a description of a random phenomenon. 
- It consists of two parts: i) defining the sample space $S$, and ii) listing the probability associated with
different events.
...
2. All possible outcomes that compose a sample space together have a probability of 1. Let $S$ denote the sample space, then:
    * $P(S) = 1$
3. If two events (here denoted by $A$ and $B$ have a joint probability of 0 (i.e., there is no overlap in their event 
spaces and they cannot both occur at the same time) then they are **disjoint** and the probability of either event occurring
...
event occurring. This event (i.e., "not occurring") is called the **complement**. 
